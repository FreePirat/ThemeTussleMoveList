...
namespace ThemeTussleMoveList
{
    public partial class Leo : Form
    {
        ...
        private void Leo_Load(object sender, EventArgs e)
        {
            LoadFirstMove();
        }
        private void LoadFirstMove()
        {
            ...
            object moveId = DataAccess.GetValue("SELECT TOP (1) MoveId FROM Move WHERE CharacterId = 3");
        ...
        private void LoadMoveDetails()
        {
            ...
            {
                $"SELECT * FROM Move WHERE MoveId = {currentId} AND CharacterId = 3",
                $@"
                SELECT 
                (
                    SELECT TOP(1) MoveId as FirstId FROM Move WHERE CharacterId = 3
                ) as FirstId,
                q.PreviousId,
                q.NextId,
                (
                    SELECT TOP(1) MoveId as LastId FROM Move WHERE CharacterId = 3 ORDER BY MoveId Desc
                ) as LastId,
                q.RowNumber
                FROM
                (
                    ...
                    FROM Move WHERE CharacterId = 3
                ) AS q
                WHERE q.MoveId = {currentId}",
                 $"SELECT COUNT(MoveId) as MoveCount FROM Move WHERE CharacterId = 3"
            };
