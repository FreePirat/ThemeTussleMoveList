...
{
    public partial class Suzie : Form
    {
        public Suzie()
        {
            InitializeComponent();
        ...
        private void Suzie_Load(object sender, EventArgs e)
        {
            LoadFirstMove();
        }
        private void LoadFirstMove()
        {
            ...
            object moveId = DataAccess.GetValue("SELECT TOP (1) MoveId FROM Move WHERE CharacterId = 2");
        ...
        private void LoadMoveDetails()
        {
            ...
            {
                $"SELECT * FROM Move WHERE MoveId = {currentId} AND CharacterId = 2",
                $@"
                SELECT 
                (
                    SELECT TOP(1) MoveId as FirstId FROM Move WHERE CharacterId = 2
                ) as FirstId,
                q.PreviousId,
                q.NextId,
                (
                    SELECT TOP(1) MoveId as LastId FROM Move WHERE CharacterId = 2 ORDER BY MoveId Desc
                ) as LastId,
                q.RowNumber
                FROM
                (
                    ...
                    FROM Move WHERE CharacterId = 2
                ) AS q
                WHERE q.MoveId = {currentId}",
                 $"SELECT COUNT(MoveId) as MoveCount FROM Move WHERE CharacterId = 2"
            };
