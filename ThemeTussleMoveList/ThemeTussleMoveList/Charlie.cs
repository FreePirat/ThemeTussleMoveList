...
namespace ThemeTussleMoveList
{
    public partial class Charlie : Form
    {
        ...
        private void Charlie_Load(object sender, EventArgs e)
        {
            LoadFirstMove();
        }
        private void LoadFirstMove()
        {
            ...
            object moveId = DataAccess.GetValue("SELECT TOP (1) MoveId FROM Move WHERE CharacterId = 4");
        ...
        private void LoadMoveDetails()
        {
            ...
            {
                $"SELECT * FROM Move WHERE MoveId = {currentId} AND CharacterId = 4",
                $@"
                SELECT 
                (
                    SELECT TOP(1) MoveId as FirstId FROM Move WHERE CharacterId = 4
                ) as FirstId,
                q.PreviousId,
                q.NextId,
                (
                    SELECT TOP(1) MoveId as LastId FROM Move WHERE CharacterId = 4 ORDER BY MoveId Desc
                ) as LastId,
                q.RowNumber
                FROM
                (
                    ...
                    FROM Move WHERE CharacterId = 4
                ) AS q
                WHERE q.MoveId = {currentId}",
                 $"SELECT COUNT(MoveId) as MoveCount FROM Move WHERE CharacterId = 4"
            };
